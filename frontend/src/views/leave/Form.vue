...
const leaveApprovalDetails = createResource({
	...
	params: { employee: employee.value.name },
	onSuccess(data) {
...
watch(
	...
	(employee_id) => {
		if (props.id && employee_id !== employee.value.name) {
...
function setFormReadOnly() {
	if (leaveApplication.value.leave_approver === employee.value.user_id) return
...
function validateForm() {
	...
	leaveApplication.value.employee = employee.value.name
}
</script>
